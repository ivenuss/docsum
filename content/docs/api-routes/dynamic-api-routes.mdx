...
description:
  You can add the dynamic routes used for pages to API Routes too. Learn how it
  works here.
---
...
API routes support [dynamic routes](/docs/routing/dynamic-routes.md), and follow
...
have an `undefined` state and `GET api/posts` will not match
...
- `pages/api/post/[...slug].js` matches `/api/post/a`, but also `/api/post/a/b`,
  `/api/post/a/b/c` and so on.

> **Note**: You can use names other than `slug`, such as: `[...param]`

Matched parameters will be sent as a query parameter (`slug` in the example) to
the page, and it will always be an array, so, the path `/api/post/a` will have
...
And in the case of `/api/post/a/b`, and any other matching path, new parameters
...
optional, the route without the parameter is also matched (`/api/post` in the
example above).
